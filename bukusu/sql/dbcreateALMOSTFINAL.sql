...
*/
--ALMOST FINAL - NEED TO UNCOMMENT USER INFO 
...
-- Columns:
--		initialSound	- Acronym of sound description
--		initialSoundDescr	- Only options currently of interest are nasal, vowel and voiceless fricative (think [s])
...
-- Columns:
--		wordID		- unique ID for the word
--		wordForm 	- word to be inserted
--		morpheme1  	- first morpheme in the word
--		morpheme2	- second morepheme in the word (if applicable)
-- 		rootID 		- root associated with the word
-- 		pos			- part of speech (noun/verb) of the word
--		subPos		- sub part of speech of the word
--		numOfSyllables - the number of syllables in the word
--		sigma1		- length of the first syllable (1 - short, 2 - long)
--		sigma2		- length of the second syllable (1 - short, 2 - long, 0 - NA)
--		initialSound - C, V, NC, or LF
--		tonalPattern - linking the tonal pattern to the part of speech of the word
--		audioID		- links the word to the audio file
...
CREATE TABLE words (
	...
	gloss varchar(500), -- definitions of single words only
	tonalPattern varchar(128),
    audioID integer REFERENCES audio(audioId)
);

-- Table: gp.modifiers
-- Columns: 
--		modifier	- element of phrase
DROP TABLE IF EXISTS modifiers;
CREATE TABLE modifiers(
	modifier varchar(20) PRIMARY KEY
);

-- Table: gp.phrasal_data
-- Columns:
--		phraseID	- unique ID for the phrase
--		phraseForm	= Bukusu translation for the phrase
--		definition	- English definition of the phrase
--		noun1,2,3	- nouns in the phrase, linked to the words table
--		verb1,2,3	- verbs in the phrase, linked to the words table
--		numModifiers - the number of modifiers in the phrase
--		modifier1,2	- the modifiers in the phrase, linked to the modifiers table
-- 		audioID		- links the phrase to the audio file
